...
export default function ProductCard() {
    function handleClick(product){
        ...
        let itemIndex=local_favs.findeIndex(localItem=>product.id===localItem.id)
        if(itemIndex>=0){
            local_favs=local_favs.filter(item=>item.id!==product.id)
        }else{
            local_favs.push({id:product.id,title:product.title,price:product.price,image:product.price})
        }
